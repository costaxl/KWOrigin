...
- (id)initWithDelegate:(id <MWPhotoBrowserDelegate>)delegate {
    if ((self = [self init])) {
        _delegate = delegate;
        _gridControllerClass = [MWGridViewController class];
...
- (id)initWithDelegateAndGridController:(id <MWPhotoBrowserDelegate>)delegate gridController:(Class)ControllerClass
{
    ...
    {
        _delegate = delegate;
        _gridControllerClass = ControllerClass;
	}
	return self;
...
- (void)viewDidLoad {
    ...
    if (_startOnGrid) _gridding = YES;
	
    if (_enableBatchLoadThumbnails)
    {
        _enableBatchLoadThumbnails = [_delegate respondsToSelector:@selector(photoBrowser:prepareThumbnails:)];
        _enableBatchLoadThumbnails = [_delegate respondsToSelector:@selector(isThumbnailsLoading:)];
    }
	// View
...
- (void)performLayout {
    ...
    if (([self.navigationController.viewControllers objectAtIndex:0] == self))
    {
        ...
        if (_gridController)
            _gridController.isModal = false;
        _isModal = false;
        if (_startOnGrid)
        {
            // done button need to back to grid
            //self.navigationItem.leftBarButtonItem = _doneButton;
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    if ([self.delegate respondsToSelector:@selector(photoBrowserWillHideView)])
    {
        
        [self.delegate photoBrowserWillHideView];
    }
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if ([self.delegate respondsToSelector:@selector(photoBrowserShowView)])
    {
        
        [self.delegate photoBrowserShowView];
    }
...
- (UIImage *)imageForPhoto:(id<MWPhoto>)photo {
	if (photo) {
		...
		if ([photo underlyingImage]) {
            ...
            //[Arthur test] test behavior of photo browser
...
- (NSInteger)numberOfPhotosInSection:(NSInteger)section
{
    if ([_delegate respondsToSelector:@selector(numberOfPhotosInPhotoBrowser:)])
    {
        return [_delegate numberOfPhotosInSection:section inPhotoBrowser:self];
    }
    return 0;
}
-(NSInteger)numberOfSection
{
    ...
    {
        return [_delegate numberOfSectionsInPhotoBrowser:self];
    }
    return 0;
}
- (id<MWPhoto>)photoAtIndex:(NSUInteger)index section:(NSInteger)Section
{
    id <MWPhoto> photo = nil;
...
- (id<MWPhoto>)thumbPhotoAtIndex:(NSUInteger)index section:(NSInteger)Section
{
    id <MWPhoto> photo = nil;
...
{
    if ([_delegate respondsToSelector:@selector(photoBrowser:titleForSection:)])
    {
        return [_delegate photoBrowser:self titleForSection:Section];
    }
    return nil;
}
...
- (void)updateNavigation {
    ...
    if (_gridController)
    {
        [self.navigationItem setTitle:[_gridController GetTitle]];
        //self.title = [_gridController GetTitle];
...
//        }
    ...
    else if (numberOfPhotos > 1)
    {
        ...
        {
            [self.navigationItem setTitle:[_delegate photoBrowser:self titleForPhotoAtIndex:_currentPageIndex]];

            //self.title = [_delegate photoBrowser:self titleForPhotoAtIndex:_currentPageIndex];
        }
        else
        {
            [self.navigationItem setTitle:[NSString stringWithFormat:@"%lu %@ %lu", (unsigned long)(_currentPageIndex+1), NSLocalizedString(@"of", @"Used in the context: 'Showing 1 of 3 items'"), (unsigned long)numberOfPhotos]];
...
- (void)showGrid:(BOOL)animated {
    ...
    if (_gridControllerClass)
    {
        _gridController = [[_gridControllerClass alloc] init];
    }
    else
    {
    ...
    _gridController.m_bBatchLoadThumbnails = _enableBatchLoadThumbnails;
    ...
    _gridController.isModal = _isModal;
    ...
    if ((self.navigationItem.leftBarButtonItem == _doneButton) ||  (self.navigationItem.leftBarButtonItem == nil))
    {
    ...
    if ([self.delegate respondsToSelector:@selector(willShowGrid)])
    {
        
        [self.delegate willShowGrid];
    }
...
- (void)hideGrid {
    ...
    NSArray* ButtonArray;
    ButtonArray = [[NSArray alloc] initWithObjects:_gridPreviousRightNavItem,  nil];
    self.navigationItem.RightBarButtonItems = ButtonArray;
//    if (_gridPreviousRightNavItem == _actionButton && _actionButton)
//    {
//        [self.navigationItem setRightBarButtonItem:_gridPreviousRightNavItem animated:YES];
//    }
    if (((_gridPreviousLeftNavItem == _doneButton) || (_gridPreviousLeftNavItem == nil))&& _doneButton)
    {
        if (!_isModal)
    ...
    _gridding = false;
    if ([self.delegate respondsToSelector:@selector(willHideGrid)])
    {
        
        [self.delegate willHideGrid];
    }
...
- (void)setControlsHidden:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    ...
    [UIView animateWithDuration:animationDuration animations:^(void) {
        ...
        // hide bottom bar
        if ([self.delegate respondsToSelector:@selector(photoBrowser:hideBottomBar:)])
        {
            
            [self.delegate photoBrowser:self hideBottomBar:hidden];
        }
...
-(void)didSelectPhotoIndex:(NSInteger)Index
{
    if ([_delegate respondsToSelector:@selector(photoBrowser:didSelectPhotoAtIndex:)])
    {
        [_delegate photoBrowser:self didSelectPhotoAtIndex:Index];
    }
...
- (BOOL)canBatchLoadThumbnail
{
    if ([self.delegate respondsToSelector:@selector(isThumbnailsLoading:)])
    {
        // Let delegate handle things
        return ![self.delegate isThumbnailsLoading:self];
        
    }
    else
        return false;
}
- (BOOL)prepareBatchLoad
{
    if ([self.delegate respondsToSelector:@selector(prepareThumbnailsLoading:)])
    {
        // Let delegate handle things
        return [self.delegate prepareThumbnailsLoading:self];
...
- (void)batchLoadThumbnail:(NSArray*)IndexPaths
{
    [self PrepareThumbnails:IndexPaths];

}

-(void)PrepareThumbnails:(NSArray*)IndexPaths
{
    if ([self.delegate respondsToSelector:@selector(photoBrowser:prepareThumbnails:)])
    {
        // Let delegate handle things
        [self.delegate photoBrowser:self prepareThumbnails:IndexPaths];
        
    }

}

- (void)TakeAction:(enum ActionType) Action with:(id)Parameter
{
    if ([self.delegate respondsToSelector:@selector(photoBrowser:action:with:)])
    {
        [self.delegate photoBrowser:self action:Action with:Parameter];
    }
}
