//
//  ShareFolderPhotoGridViewController.m
//
...
@interface ShareFolderPhotoGridViewController ()
{
    UIToolbar *m_ToolBar;
    ...
    UIBarButtonItem* m_SelectModeButton, *m_DBDownloadButton, *m_SelectionModeReturn, *m_PCDownloadButton;
...
- (void)DBDownloadAction;
- (void)PCDownloadAction;
...
@implementation ShareFolderPhotoGridViewController
...
- (void)viewDidLoad {
    ...
    NSString *DBDownloadImagePathFormat;
    NSString *PCDownloadImagePathFormat;

    NSString *selectModeReturnImagePathFormat;
    if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"7"))
    {
        selectImagePathFormat = @"kwDelete_25.png";
        DBDownloadImagePathFormat = @"AccessDropbox_25.png";
        PCDownloadImagePathFormat = @"174-imac.png";
        selectModeReturnImagePathFormat = @"03-loopback.png";
    }
    else
    {
    ...
    UIImage* DBDownloadImage = [UIImage imageNamed:DBDownloadImagePathFormat];
    m_DBDownloadButton = [[UIBarButtonItem alloc] initWithImage:DBDownloadImage style:UIBarButtonItemStylePlain target:self action:@selector(DBDownloadAction)];
    UIImage* PCDownloadImage = [UIImage imageNamed:PCDownloadImagePathFormat];
    m_PCDownloadButton = [[UIBarButtonItem alloc] initWithImage:PCDownloadImage style:UIBarButtonItemStylePlain target:self action:@selector(PCDownloadAction)];
    ...
    if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"7"))
    {
        ...
        shareImagePathFormat = @"12-eye.png";
        downloadImagePathFormat = @"13-target.png";
    }
    else
    {
        //arrowPathFormat = @"MWPhotoBrowser.bundle/images/UIBarButtonItemArrow%@.png";
...
- (void)performLayout
{
    ...
    if (!m_bSelection)
    {
        ButtonArray = [[NSArray alloc] initWithObjects:m_SelectModeButton, m_PCDownloadButton, m_DBDownloadButton,  nil];
...
- (void)DBDownloadAction
{
    [self.browser TakeAction:kAction_DBDownload with:nil];
}
- (void)PCDownloadAction
{
    [self.browser TakeAction:kAction_PCDownload with:nil];
}
