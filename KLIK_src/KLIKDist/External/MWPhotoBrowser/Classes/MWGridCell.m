...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        // Image
        _imageShareView = [UIImageView new];
        [_imageShareView setImage:[UIImage imageNamed:@"201-Users-Group-icon.png"]];
        _imageShareView.frame = CGRectMake(0, 0, 40.0f, 40.0f);
        _imageShareView.contentMode = UIViewContentModeScaleAspectFit;
        _imageShareView.clipsToBounds = YES;
        _imageShareView.autoresizesSubviews = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
        _imageShareView.hidden = YES;
        _imageShareView.alpha = 0.6;
        [_imageShareView setBackgroundColor:[UIColor clearColor]];
        [self addSubview:_imageShareView];
...
- (void)layoutSubviews {
    ...
    _imageShareView.frame = CGRectMake(self.bounds.size.width - _imageShareView.frame.size.width - 0,
                                       self.bounds.size.height - _imageShareView.frame.size.height - 0, _imageShareView.frame.size.width, _imageShareView.frame.size.height);
}
...
- (void)setPhoto:(id <MWPhoto>)photo {
    ...
    if (_photo)
    {
        ...
        if (([_photo respondsToSelector:@selector(Privacy)]) && ([_photo Privacy]==1))
        {
            //share
            _imageShareView.hidden = NO;
